...
function initRDS(Hash,Q,Code,Prev,Next,cut){
 for(var i=0,j,l=Code.length-1,a,b,c=-1,pair;i<l;i++,c=a)
	if(pair=locatePair(Hash,a=Code[i],b=Code[i+1]))
	 Next[Prev[i]=pair.pos]=pair.pos=i,
	 cut===1&&a===b&&a===c&&i-1===j||incrementPair(Q,pair,j=i),
	 cut<2||a^b||a^c||i++;
	else createPair(Hash,Q,a,b,j=i);
 resetPQ(Hash,Q,1)}
...
function RePair(A,cut){
 ...
 for(var x=-1>>>0,y=128,B=0,N=0,b=0,d=0,l=0,m,n=1/0,cl=0,nc=0,nl=A.length,o=nl,Hash=[],Code=Array(o),Prev=Array(o),Next=Array(o),Left=[],Right=[],O=[],Q=[],T=[],E=[],F=[],Y=[];y;)Y[--y]=4096/(y+128)|0;
 ...
 Hash.hN=15;initRDS(Hash,Q,Code,Prev,Next,cut&3);
 for(;m=getMaxPair(Q);replace(Hash,Q,Code,Prev,Next,m,n)&&n++)
	Left[n]=m.left,Right[n]=m.right;
 for(m=n=l=Q.length=Next.length=0;n<nl;)Code[n]>-1?Code[m++]=Code[n++]:n=Prev[n];
 for(;m>>++l;);
 eB(5,l);eB(--l,m^1<<l);
 for(l=nc;l-1>>++cl;);
 T.e=function(L,R,c,m,n){
	if(this[c]>-1){
	 eb(b,b=1,F);
	 if(c<l)eb(d+3,d=0,F),eb2(cl,c,Q);
	 else{
		for(c=this[c]-l;nc>>nl;)nl++;
		eb(d+3,d=1,F);
	 ...
	 this.e(L,R,L[c]),
	 this.e(L,R,R[c]),
	 eb(b,b=0,F),this[c]=l+nc++};
 for(nc=nl=n=0;n<m;b=2)T.e(Left,Right,Code[n++]),eb(b,0,F);
 for(n=5;n--;y=y<<8>>>0)
	if((y^0xff000000)>>>0>0xffffff){
...
function DePair(A){
 ...
 for(var a=0,b=0,c,d,e=0,f=0,l,m,n=0,cl=0,o,r,s,x=-1>>>0,y=128,O=[],L=[],R=[],S=[],T=[],Y=[],D=[],E=[],F=[];y;)Y[--y]=4096/(y+128)|0;
 for(;a<4;)y=(y<<8|A[a++])>>>0;
 ...
 for(r=dB(--r)|1<<r,m=n;r--;)
	for(e=s=0;;)
	 if(db(F,b)){b=1;e++;
		if(f=db(F,f+3)){
		 for(d=n-m;d>>l;)l++;
		 if((c=db2(E,l-1))>=(d=(1<<l)-d))c+=c+dB(1)-d;c+=m
		}else c=db2(D,cl);
		O.e(L,R,S[s++]=c)}
